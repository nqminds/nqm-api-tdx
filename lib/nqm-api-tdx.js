  ...
  return response.json().then(function (json) {
    if (response.ok) {
      ...
      if (json.error) {
        ...
        return _bluebird2.default.reject(handleError(source, { code: response.status, message: JSON.stringify(failure) }));
      } else {
        return _bluebird2.default.reject(handleError(source, { code: response.status, message: JSON.stringify(json) }));
      }
    }
  });
};
