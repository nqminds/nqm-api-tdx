  ...
  return response.json().then(function (json) {
    if (response.ok) {
      ...
      if (json.error) {
        ...
        return _bluebird2.default.reject(handleError(source, json.error));
      } else {
        return _bluebird2.default.reject(handleError(source, JSON.stringify({ code: response.statusCode, message: json })));
      }
    }
  });
};
