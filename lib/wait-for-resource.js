      ...
      if (err) {
        if (err.name !== "TDXApiError") {
          ...
          try {
            ...
            if (failure.code === "NotFoundError") {
              // Ignore resource not found errors here, they are probably caused by waiting for the projections to
              // catch up (esp. in debug environments) by falling through we will still be limited by the retry count,
